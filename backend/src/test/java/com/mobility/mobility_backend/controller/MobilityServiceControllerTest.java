...
class MobilityServiceControllerTest {
    ...
    void whenCreateMobilityService_thenSuccess() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/mobility-services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestDTO)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.serviceId").value(1L))
                .andExpect(jsonPath("$.name").value("Vélib"))
    ...
    void whenGetAllMobilityServices_thenSuccess() throws Exception {
        ...
        when(mobilityServiceService.getAllMobilityServices())
                .thenReturn(services);

        // When & Then
        mockMvc.perform(get("/api/mobility-services"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0].name").value("Vélib"))
    ...
    void whenGetMobilityServiceById_thenSuccess() throws Exception {
        ...
        when(mobilityServiceService.getMobilityServiceById(1L))
                .thenReturn(Optional.of(service));

        // When & Then
        mockMvc.perform(get("/api/mobility-services/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.serviceId").value(1L))
                .andExpect(jsonPath("$.name").value("Vélib"));
    }
    ...
    void whenGetNonExistingMobilityServiceById_thenNotFound() throws Exception {
        // Given
        when(mobilityServiceService.getMobilityServiceById(99L))
                .thenReturn(Optional.empty());

        // When & Then
        mockMvc.perform(get("/api/mobility-services/99"))
    ...
    void whenGetMobilityServiceByName_thenSuccess() throws Exception {
        ...
        when(mobilityServiceService.getMobilityServiceByName("Vélib"))
                .thenReturn(Optional.of(service));

        // When & Then
        mockMvc.perform(get("/api/mobility-services/name/Vélib"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.serviceId").value(1L))
                .andExpect(jsonPath("$.name").value("Vélib"));
    }
    ...
    void whenGetNonExistingMobilityServiceByName_thenNotFound() throws Exception {
        // Given
        when(mobilityServiceService.getMobilityServiceByName("NonExistent"))
                .thenReturn(Optional.empty());

        // When & Then
        mockMvc.perform(get("/api/mobility-services/name/NonExistent"))
                .andExpect(status().isNotFound());
    }
    ...
    void whenDeleteExistingMobilityService_thenSuccess() throws Exception {
        // Given
        when(mobilityServiceService.deleteMobilityService(1L))
                .thenReturn(true);

        // When & Then
        mockMvc.perform(delete("/api/mobility-services/1"))
    ...
    void whenDeleteNonExistingMobilityService_thenNotFound() throws Exception {
        // Given
        when(mobilityServiceService.deleteMobilityService(99L))
                .thenReturn(false);

        // When & Then
        mockMvc.perform(delete("/api/mobility-services/99"))
                .andExpect(status().isNotFound());
    }
}
