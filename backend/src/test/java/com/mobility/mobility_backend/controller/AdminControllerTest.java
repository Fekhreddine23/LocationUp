...
class AdminControllerTest {
	...
	void whenGetAllAdmins_thenReturnAdmins() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2))
				.andExpect(jsonPath("$[0].adminId").value(1)).andExpect(jsonPath("$[0].username").value("admin1"))
				.andExpect(jsonPath("$[1].adminId").value(2)).andExpect(jsonPath("$[1].username").value("admin2"));
	}
	...
	void whenGetExistingAdminById_thenReturnAdmin() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/1")).andExpect(status().isOk()).andExpect(jsonPath("$.adminId").value(1))
	...
	void whenGetNonExistingAdminById_thenReturnNotFound() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/999")).andExpect(status().isNotFound());
	}
	...
	void whenGetAdminByExistingUsername_thenReturnAdmin() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/username/admin1")).andExpect(status().isOk())
				.andExpect(jsonPath("$.username").value("admin1"));
	}
	...
	void whenGetAdminByNonExistingUsername_thenReturnNotFound() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/username/unknown")).andExpect(status().isNotFound());
	}
	...
	void whenCreateValidAdmin_thenReturnCreated() throws Exception {
		...
		// When & Then
		mockMvc.perform(post("/api/admins").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(createAdminDTO))).andExpect(status().isCreated())
				.andExpect(jsonPath("$.adminId").value(1)).andExpect(jsonPath("$.username").value("newadmin"));
	}
	...
	void whenCreateAdminWithExistingUsername_thenReturnBadRequest() throws Exception {
		...
		// When & Then
		mockMvc.perform(post("/api/admins").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(createAdminDTO))).andExpect(status().isBadRequest())
	...
	void whenUpdateExistingAdmin_thenReturnUpdatedAdmin() throws Exception {
		...
		when(adminService.updateAdmin(eq(1), any(AdminDTO.class))).thenReturn(Optional.of(updatedAdmin));

		// When & Then
		mockMvc.perform(put("/api/admins/1").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(adminDTO))).andExpect(status().isOk())
				.andExpect(jsonPath("$.adminId").value(1)).andExpect(jsonPath("$.username").value("updated"));
	}
	...
	void whenUpdateNonExistingAdmin_thenReturnNotFound() throws Exception {
		...
		when(adminService.updateAdmin(eq(999), any(AdminDTO.class))).thenReturn(Optional.empty());

		// When & Then
		mockMvc.perform(put("/api/admins/999").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(adminDTO))).andExpect(status().isNotFound());
	}
	...
	void whenDeleteNonExistingAdmin_thenReturnNotFound() throws Exception {
		...
		// When & Then
		mockMvc.perform(delete("/api/admins/999")).andExpect(status().isNotFound());
	}
	...
	void whenCheckExistingUsername_thenReturnTrue() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/check-username/existing")).andExpect(status().isOk())
	...
	void whenCheckNonExistingUsername_thenReturnFalse() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/check-username/nonexisting")).andExpect(status().isOk())
	...
	void whenCheckExistingEmail_thenReturnTrue() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/check-email/existing@test.com")).andExpect(status().isOk())
				.andExpect(content().string("true"));
	}
	...
	void whenCheckNonExistingEmail_thenReturnFalse() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/api/admins/check-email/nonexisting@test.com")).andExpect(status().isOk())
				.andExpect(content().string("false"));
	}
