...
public class DebugAuthController {
	...
	public ResponseEntity<?> testPassword(@RequestBody Map<String, String> request) {
		...
		if (matches) {
			...
			return ResponseEntity.ok(Map.of("token", token, "username", user.getUsername(), "role",
					user.getRole().name(), "passwordMatch", true));
		} else {
			return ResponseEntity.status(403).body(Map.of("error", "Password mismatch", "passwordMatch", false));
		}
	}
